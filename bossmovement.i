...
void initBoss(BOSS* boss, int x, int y, int width, int height) {
    ...
    boss->width = width;
    boss->height = height;
    boss->behaviorState = 0;
    boss->behaviorTimer = 60;
...
void updateBoss(BOSS* boss, SPRITE* player) {
    ...
    if (boss->behaviorTimer <= 0) {
        ...
        boss->behaviorState = rand() % 4;


        boss->behaviorTimer = 60 + (rand() % 60);
    }
    ...
    switch (boss->behaviorState) {
        ...
        case 1:
            ...
            if (distanceSquared > 900 && distanceSquared < 2500) {
    ...
    if (boss->x + boss->width > 256) boss->x = 256 - boss->width;
    if (boss->y + boss->height > 256) boss->y = 256 - boss->height;
}
